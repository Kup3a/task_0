...
public class FileUserStore implements UserStore {
    ...
    public boolean isUserExist(String login) {
        ...
        try {
            String storage = fw.readFile("D:\\Projects\\IdeaProjects\\technotrackChat\\src\\storage.txt");
            HashMap<String, User> map = new HashMap();
            String[] s = storage.split("\\s");
            for (int i = 0; i < s.length / 4; i++) {
                User t = new User(s[4*i], s[4*i + 1], s[4*i + 2], s[4*i + 3]);
                map.put(t.getLogin(), t);
                if (map.get(t.getLogin()) == null) {
                    return false;
                } else {
                    return true;
                }
    ...
    public void addUser(User user) {
        ...
        try {
            fw.writeToFile(user.getLogin() + " " + HashClass.createHash(user.getPassword()), "D:\\Projects\\IdeaProjects\\technotrackChat\\src\\storage.txt");
        } catch (FileNotFoundException e) {
            e.printStackTrace();
